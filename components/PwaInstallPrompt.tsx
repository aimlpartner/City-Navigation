...
export function PwaInstallPrompt() {
  ...
  return (
    <>
      ...
      {isOffline && (
        <div className="fixed top-2 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#102a20]/95 backdrop-blur-md border border-emerald-500/40 text-emerald-300 text-xs font-semibold shadow-lg animate-bounce">
      ...
      {installSuccess && (
        <div className="fixed top-3 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-[#143428] border border-emerald-400/50 text-white text-xs font-bold shadow-2xl animate-in fade-in slide-in-from-top duration-300">
      ...
      {!isInstalled && showBanner && (
        <div className="fixed bottom-4 left-3 right-3 sm:left-auto sm:right-6 sm:max-w-sm z-50 bg-[#143428]/95 backdrop-blur-xl border border-emerald-400/30 rounded-2xl p-3.5 sm:p-4 text-white shadow-2xl transition-all duration-300">
          <div className="flex items-start gap-3">
            <div className="w-12 h-12 rounded-xl bg-[#102a20] border border-emerald-400/30 flex items-center justify-center shrink-0 p-1.5 shadow-inner">
              <Image
                src="/icons/icon.svg"
            ...
            <div className="flex-1 min-w-0 pr-1">
              <div className="flex items-center justify-between">
                <h4 className="font-extrabold text-sm text-white tracking-tight flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] font-bold px-1.5 py-0.2 rounded bg-emerald-400/20 text-emerald-300">PWA</span>
                </h4>
                <button
                  onClick={dismissBanner}
                  className="text-emerald-300/60 hover:text-emerald-300 p-0.5 rounded-lg transition cursor-pointer"
              ...
              <div className="flex items-center gap-2 mt-2.5">
                <button
                  ...
                  className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#5ee9b5] hover:bg-[#4ade80] text-[#0d211a] text-xs font-bold transition shadow-xs cursor-pointer"
                >
                ...
                <button
                  onClick={dismissBanner}
                  className="px-2.5 py-1.5 rounded-lg bg-white/10 hover:bg-white/15 text-white text-xs font-medium transition cursor-pointer"
                >
                  Later
                </button>
              </div>
      ...
      {showUniversalModal && (
        <div
          className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-end sm:items-center justify-center p-3 sm:p-4 animate-in fade-in duration-200"
          onClick={() => setShowUniversalModal(false)}
        >
          <div
            className="bg-[#143428] border border-emerald-400/30 rounded-3xl p-5 sm:p-6 max-w-lg w-full text-white shadow-2xl relative max-h-[90vh] overflow-y-auto"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={() => setShowUniversalModal(false)}
              className="absolute top-4 right-4 text-emerald-200/60 hover:text-white p-1.5 rounded-full bg-white/10 hover:bg-white/20 transition cursor-pointer"
            ...
            <div className="flex items-center gap-3.5 mb-5">
              <div className="w-14 h-14 rounded-2xl bg-[#102a20] border border-emerald-400/40 p-2 shrink-0 shadow-lg">
                <Image
                  src="/icons/icon.svg"
                  alt="MetroNav"
                  width={56}
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="px-2 py-0.5 rounded-full bg-[#5ee9b5]/20 text-[#5ee9b5] text-[11px] font-bold">
                    Offline Ready
                  </span>
                </div>
            ...
            {deferredPrompt && (
              <div className="mb-5 p-3.5 rounded-2xl bg-emerald-500/15 border border-emerald-400/40 flex items-center justify-between gap-3">
                ...
                <button
                  onClick={handleInstallClick}
                  className="px-4 py-2 rounded-xl bg-[#5ee9b5] text-[#0d211a] font-bold text-xs hover:bg-[#4ade80] transition shrink-0 flex items-center gap-1.5 shadow-md cursor-pointer"
                >
                  <Download className="w-3.5 h-3.5" />
            ...
            <div className="flex items-center gap-1.5 p-1 rounded-2xl bg-[#0e241c] border border-emerald-500/20 mb-4">
              <button
                type="button"
            ...
            <div className="space-y-2.5 bg-[#0e241c] rounded-2xl p-4 border border-emerald-500/20 text-xs text-emerald-100 mb-4">
              {activePlatformTab === 'android' && (
                <>
                  <div className="flex items-start gap-3">
            ...
            <div className="grid grid-cols-2 gap-2 text-[11px] text-emerald-200/90 mb-4">
